...
const insta = (url) => new Promise((resolve, reject) => {
    console.log('Get metadata from =>', url)
    fetchJson('http://api.fdci.se/sosmed/insta.php?url=' + url)
        .then((result) => {
        if (result.status != true) return resolve(result.status)
        })
        .catch((err) => {
            console.error(err)
            reject(err)
...
const ytmp3 = (url) => new Promise((resolve, reject) => {
    console.log('Get metadata from =>', url)
    fetchJson('https://api.kry9ton.tech/v1/yta?url='+ url)
        .then((result) => {
...
const ytmp4 = (url) => new Promise((resolve, reject) => {
    console.log('Get metadata from =>', url)
    fetchJson('https://api.kry9ton.tech/v1/ytv?url='+ url)
        .then((result) => {
            if (result.status != 200) return resolve(result.status)
        resolve(result)
        }).catch((err) => {
